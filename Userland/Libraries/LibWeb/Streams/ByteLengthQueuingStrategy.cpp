    ...
    , m_high_water_mark(high_water_mark)
{
    ...
    auto function = JS::NativeFunction::create(global_objects_relevant_realm, move(steps), 1, "size", &global_objects_relevant_realm);
    ...
    window_or_worker.set_byte_length_queuing_strategy_size_function(MUST(global_object.heap().allocate<WebIDL::CallbackType>(global_objects_relevant_realm, function, HTML::relevant_settings_object(global_object))));
}

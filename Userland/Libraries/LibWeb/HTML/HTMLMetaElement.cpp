...
{
    auto value = attribute(HTML::AttributeNames::http_equiv.to_deprecated_fly_string());
...
void HTMLMetaElement::inserted()
{
    ...
    if (http_equiv.has_value()) {
        ...
        case HttpEquivAttributeState::Refresh: {
            ...
            auto input = attribute(AttributeNames::content.to_deprecated_fly_string());
            if (input.is_empty())
                break;
        ...
        default:
            dbgln("FIXME: Implement '{}' http-equiv state", attribute(AttributeNames::http_equiv.to_deprecated_fly_string()));
            break;
        }
    }
}

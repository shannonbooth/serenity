...
static ErrorOr<String> apply_text_transform(String const& string, CSS::TextTransform text_transform)
{
    ...
    case CSS::TextTransform::Capitalize: {
        return TRY(Unicode::to_unicode_titlecase_full(string, {}, Unicode::TrailingCodePointTransformation::PreserveExisting));
    }
...
void TextNode::compute_text_for_rendering()
{
    ...
    if (dom_node().is_password_input()) {
        m_text_for_rendering = MUST(String::repeated('*', data.bytes().size()));
        return;
    }
    ...
    if (data.bytes().size() == 1) {
        if (is_ascii_space(data.bytes_as_string_view()[0])) {
    ...
    for (auto c : data.bytes_as_string_view()) {
        if (is_ascii_space(c)) {
            contains_space = true;
            break;
        }
    ...
    StringBuilder builder(data.bytes().size());
    size_t index = 0;

    auto skip_over_whitespace = [&index, &data] {
        while (index < data.bytes().size() && is_ascii_space(data.bytes_as_string_view()[index]))
            ++index;
    };

    while (index < data.bytes().size()) {
        if (is_ascii_space(data.bytes_as_string_view()[index])) {
            builder.append(' ');
            ++index;
            skip_over_whitespace();
        } else {
            builder.append(data.bytes_as_string_view()[index]);
            ++index;
        }
    }

...
JS::ThrowCompletionOr<JS::NonnullGCPtr<JS::Object>> ImageConstructor::construct(FunctionObject&)
{
    ...
    if (vm.argument_count() > 0) {
        ...
        MUST(image_element->set_attribute(HTML::AttributeNames::width.to_deprecated_fly_string(), DeprecatedString::formatted("{}", width)));
    }
    ...
    if (vm.argument_count() > 1) {
        ...
        MUST(image_element->set_attribute(HTML::AttributeNames::height.to_deprecated_fly_string(), DeprecatedString::formatted("{}", height)));
    }

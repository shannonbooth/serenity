...
CSSNamespaceRule::CSSNamespaceRule(JS::Realm& realm, Optional<String> const& prefix, String namespace_uri)
    : CSSRule(realm)
    , m_namespace_uri(move(namespace_uri))
    , m_prefix(prefix.value_or(String {}))
{
}

JS::NonnullGCPtr<CSSNamespaceRule> CSSNamespaceRule::create(JS::Realm& realm, Optional<String> const& prefix, String namespace_uri)
{
    return realm.heap().allocate<CSSNamespaceRule>(realm, realm, prefix, move(namespace_uri));
}

...
CSSRule* Parser::convert_to_rule(NonnullRefPtr<Rule> rule)
{
    if (rule->is_at_rule()) {
        ...
        if (rule->at_rule_name().equals_ignoring_ascii_case("namespace"sv)) {
            ...
            if (token.is(Token::Type::Ident)) {
                prefix = token.token().ident().to_string();
                token_stream.skip_whitespace();
            ...
            if (token.is(Token::Type::String)) {
                namespace_uri = MUST(String::from_utf8(token.token().string())); // FIXME: string() should not return a StringView
            ...
            return CSSNamespaceRule::create(m_context.realm(), prefix, move(namespace_uri));
        }
        ...
        if (declaration.name().equals_ignoring_ascii_case("font-family"sv)) {
            ...
            for (size_t i = 0; i < declaration.values().size(); ++i) {
                ...
                if (part.is(Token::Type::String)) {
                    ...
                    font_family_parts.append(MUST(String::from_utf8(part.token().string())));
                    continue;
                }
                if (part.is(Token::Type::Ident)) {
                    ...
                    font_family_parts.append(part.token().ident().to_string());
                    continue;
                }

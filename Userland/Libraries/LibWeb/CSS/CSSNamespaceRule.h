...
public:
    [[nodiscard]] static JS::NonnullGCPtr<CSSNamespaceRule> create(JS::Realm&, FlyString prefix, FlyString namespace_uri);

    virtual ~CSSNamespaceRule() = default;

    void set_namespace_uri(FlyString value) { m_namespace_uri = move(value); }
    FlyString namespace_uri() const { return m_namespace_uri; }
    void set_prefix(FlyString value) { m_prefix = move(value); }
    FlyString prefix() const { return m_prefix; }
    virtual Type type() const override { return Type::Namespace; }

private:
    CSSNamespaceRule(JS::Realm&, FlyString prefix, FlyString namespace_uri);

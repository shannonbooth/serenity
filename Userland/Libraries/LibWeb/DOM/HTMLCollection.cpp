...
Element* HTMLCollection::named_item(FlyString const& name_) const
{
    ...
    if (auto it = elements.find_if([&](auto& entry) { return entry->attribute(HTML::AttributeNames::id.to_deprecated_fly_string()) == name; }); it != elements.end())
        return *it;
...
Vector<DeprecatedString> HTMLCollection::supported_property_names() const
{
    ...
    for (auto& element : elements) {
        ...
        if (element->has_attribute(HTML::AttributeNames::id)) {
            auto id = element->attribute(HTML::AttributeNames::id.to_deprecated_fly_string());
        ...
        if (element->namespace_() == Namespace::HTML && element->has_attribute(HTML::AttributeNames::name)) {
            auto name = element->attribute(HTML::AttributeNames::name.to_deprecated_fly_string());

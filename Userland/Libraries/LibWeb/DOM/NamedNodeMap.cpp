...
Attr* NamedNodeMap::get_attribute_ns(Optional<FlyString> namespace_, FlyString const& local_name, size_t* item_index)
{
    return const_cast<Attr*>(const_cast<NamedNodeMap const*>(this)->get_attribute_ns(move(namespace_), local_name, item_index));
}

// https://dom.spec.whatwg.org/#concept-element-attributes-get-by-namespace
Attr const* NamedNodeMap::get_attribute_ns(Optional<FlyString> namespace_, FlyString const& local_name, size_t* item_index) const
{
    if (item_index)
        *item_index = 0;

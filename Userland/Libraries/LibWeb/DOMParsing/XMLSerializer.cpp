    ...
    for (size_t attribute_index = 0; attribute_index < element.attributes()->length(); ++attribute_index) {
        ...
        if (require_well_formed == RequireWellFormed::Yes) {
            auto local_name_set_iterator = local_name_set.find_if([&attribute](LocalNameSetEntry const& entry) {
                return entry.namespace_uri == attribute->namespace_uri()->to_deprecated_fly_string() && entry.local_name == attribute->local_name().to_deprecated_fly_string(); // FIXME
            });
        ...
        auto const& new_attribute_namespace = attribute->namespace_uri();
        (void)new_attribute_namespace;
        ...
        if (!deprecated_attribute_namespace.is_null()) {

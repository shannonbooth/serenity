...
ThrowCompletionOr<NonnullGCPtr<Duration>> difference_date(VM& vm, CalendarMethodsRecord const& calendar_record, PlainDate const& one, PlainDate const& two, Object const& options)
{
...
ThrowCompletionOr<NonnullGCPtr<Duration>> difference_temporal_plain_date(VM& vm, DifferenceOperation operation, PlainDate& temporal_date, Value other_value, Value options)
{
    ...
    // FIXME: Calendar type is wrong!
    auto calendar_record = TRY(create_calendar_methods_record(vm, NonnullGCPtr<Object> { temporal_date.calendar() }, Vector<CalendarMethod> { CalendarMethod::DateAdd, CalendarMethod::DateUntil }));

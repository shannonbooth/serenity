...
ThrowCompletionOr<NonnullGCPtr<Duration>> calendar_date_until(VM&, CalendarMethodsRecord const&, Value one, Value two, Object const& options);
...
ThrowCompletionOr<Optional<CalendarMethodsRecord>> create_calendar_methods_record_from_relative_to(VM&, PlainDate*, ZonedDateTime*, Vector<CalendarMethod> const&);

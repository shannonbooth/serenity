...
ThrowCompletionOr<Value> get_by_id(VM&, FlyString const& property, Value base_value, Value this_value, PropertyLookupCache&);
ThrowCompletionOr<Value> get_by_value(VM&, Value base_value, Value property_key_value);
ThrowCompletionOr<Value> get_global(Bytecode::Interpreter&, FlyString const& identifier, GlobalVariableCache&);
...
ThrowCompletionOr<void> set_variable(VM&, FlyString const&, Value, Op::EnvironmentMode, Op::SetVariable::InitializationMode);
...
ThrowCompletionOr<Value> get_variable(Bytecode::Interpreter&, FlyString const& name, EnvironmentVariableCache&);

struct CalleeAndThis {
    Value callee;
    Value this_value;
};
ThrowCompletionOr<CalleeAndThis> get_callee_and_this_from_environment(Bytecode::Interpreter&, FlyString const& name, u32 cache_index);
...
ThrowCompletionOr<void> create_variable(VM&, FlyString const& name, Op::EnvironmentMode, bool is_global, bool is_immutable, bool is_strict);
